...
namespace Aspose.App.Live.Demos.UI.Models.cad
{
	///<Summary>
	/// AsposeCadConversion to convert cad file to other format
	///</Summary>
	public class AsposeCadConversion : AppsBase
    {        
		...
		public Response ConvertCadToPdf(string fileName, string folderName, string outputType)
        {
            return  ProcessTask(fileName, folderName, ".pdf", false,  false, delegate (string inFilePath, string outPath, string zipOutFolder)
            {
                ...
                {   
                    ...
                    {
                        pdfSaveOptions.CorePdfOptions.Compliance = Aspose.CAD.ImageOptions.PdfCompliance.PdfA1b;
                    }
                    else if (outputType == "pdfa_1a")
                    {
                        pdfSaveOptions.CorePdfOptions.Compliance = Aspose.CAD.ImageOptions.PdfCompliance.PdfA1a;
                    }
                    else if (outputType == "pdf_15")
                    {
                        pdfSaveOptions.CorePdfOptions.Compliance = Aspose.CAD.ImageOptions.PdfCompliance.Pdf15;
                    }                    
        ...
        {          
            return  ProcessTask(fileName, folderName, ".tiff", false,  false, delegate (string inFilePath, string outPath, string zipOutFolder)
            {
                using (Aspose.CAD.Image image = Aspose.CAD.Image.Load(inFilePath))
                {
                    Aspose.CAD.ImageOptions.TiffOptions tiffSaveOptions = new Aspose.CAD.ImageOptions.TiffOptions(Aspose.CAD.FileFormats.Tiff.Enums.TiffExpectedFormat.TiffJpegRgb);
